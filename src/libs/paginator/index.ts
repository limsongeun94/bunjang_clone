...
class Paginator {
  ...
  getItems(): Items {
    ...
    if (this.options.windowMode === "SLIDING") {
      const halfSideSize =
        (this.options.windowSize - (this.options.windowSize % 2)) / 2;
      left =
        this.state.current - halfSideSize > 0
          ? this.state.current + halfSideSize > this.state.pages &&
            this.state.pages > this.options.windowSize
      ...
      const currentWindow = Math.floor(
        (this.state.current - 1) / this.options.windowSize
      );
      left = currentWindow * this.options.windowSize + 1;
      right = Math.min(
        (currentWindow + 1) * this.options.windowSize,
        this.state.pages
      );
    }
