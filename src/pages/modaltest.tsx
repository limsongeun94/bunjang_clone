...
export default ({ data, user }: IndexProps) => {
  ...
  return (
    <TestLayout categories={data.categories}>
      <TestModal show={showMyModal}>
        <div className="w-[1024px] mx-auto  bg-white h-auto">
          나만의 모달
          <button onClick={() => setShowMyModal(false)}>닫는다</button>
        </div>
      </TestModal>
      <div className="text-center">
        <button onClick={() => setShowMyModal(true)}>모달을 띄운다.</button>
      </div>
